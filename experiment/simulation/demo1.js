...
jsPlumb.ready(function () {
    ...
    document.getElementById("check-button").addEventListener("click", function () {
        ...
        if (is_connected_1_12 && is_connected_2_13 && is_connected_3_14 && is_connected_4_18 && is_connected_5_19 && is_connected_6_18 && is_connected_7_8 && is_connected_9_15 && is_connected_10_18 && is_connected_11_17 && is_connected_16_19 && is_connected_17_20 && is_connected_21_24 && is_connected_22_25 && is_connected_23_26 && !unallowed_connection_present) {
            alert("Alert ! Correct connections proceed to take reading.");
    ...
    if (mcboffstate == true) {
        ...
        if (isrotating == false) {
            if (rotoroffstate == true) {
                ...
                isrotating = true;
                rotoroffstate = true;
                document.getElementById('cirmover2').style.animation = "rotation 0s";
                document.getElementById("graph").disabled = true;
...
addToTable.addEventListener('click', () => {
    ...
    if (current_readings != rangeMeter.value) {
        current_readings = rangeMeter.value;
        readings = true;
    }
    if (readings) {
        if (clickcounter < 10) {
...
function drawgraph() {
    if (clickcounter < 7) {
        alert("Alert ! Please take atleast 6 readings in the table");
    }
    else {
        var data = [trace1];
        var layout = {
            hovermode: true,
            title: {
                text: "<b>Current (A) vs Speed (RPM)</b>"
            },
            margin:{
                r: 20,
                t: 80,
